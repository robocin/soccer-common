...
void TestParameters::test_ParameterType_eval_WithInvalidParameters_ShouldReturnNullopt() {
...
void TestParameters::test_ParameterType_eval_WithValidParameters_ShouldWork() {
  ...
  /* QString (trivial) */ {
    ...
    QCOMPARE(ParameterType<QString>::eval("Barriguinha"), QString("Barriguinha"));
  }
}
...
void TestParameters::test_Text_WithValidParameters_ShouldConstruct() {
  ...
  /* testing QString */ {
    ...
    QCOMPARE(text.payload(),
             Utils::quoted(Parameters::Detail::Regex) + ": " +
...
void TestParameters::test_File_WithValidParameters_ShouldConstruct() {
  ...
  /* testing QString */ {
    ...
    QCOMPARE(text.description(), description);
    QCOMPARE(text.payload(),
             Utils::quoted(Parameters::Detail::Filter) + ": " + Utils::quoted("Json (*.json)") +
                 ", " + Utils::quoted(Parameters::Detail::DefaultDirectory) + ": " +
                 Utils::quoted("."));
    QCOMPARE(text.isChoosable(), false);
    ...
    QCOMPARE(text.update("not/empty/not_empty.file"), false);
    QCOMPARE(string, "");
  }
}
...
void TestParameters::test_Directory_WithValidParameters_ShouldConstruct() {
  ...
  /* testing QString */ {
    ...
    QCOMPARE(text.description(), description);
    QCOMPARE(text.payload(),
             Utils::quoted(Parameters::Detail::Options) + ": " +
                 Utils::quoted(MagicEnum::name(QFileDialog::Option::ReadOnly)) + ", " +
                 Utils::quoted(Parameters::Detail::DefaultDirectory) + ": " + Utils::quoted("."));
    QCOMPARE(text.isChoosable(), false);
...
void TestParameters::test_SpinBox_WithValidParameters_ShouldConstruct() {
  ...
  /* testing short */ {
    ...
    QCOMPARE(spinBox.payload(),
             Utils::quoted(Parameters::Detail::MinValue) + ": " + QString::number(0) + ", " +
                 Utils::quoted(Parameters::Detail::MaxValue) + ": " + QString::number(10));
  ...
  /* testing int */ {
    ...
    QCOMPARE(spinBox.description(), description);
    QCOMPARE(spinBox.payload(),
             Utils::quoted(Parameters::Detail::MinValue) + ": " + QString::number(-10) + ", " +
                 Utils::quoted(Parameters::Detail::MaxValue) + ": " + QString::number(20));
    QCOMPARE(spinBox.isChoosable(), false);
  ...
  /* testing long */ {
    ...
    QCOMPARE(spinBox.description(), description);
    QCOMPARE(spinBox.payload(),
             Utils::quoted(Parameters::Detail::MinValue) + ": " + QString::number(-20) + ", " +
                 Utils::quoted(Parameters::Detail::MaxValue) + ": " + QString::number(30));
    QCOMPARE(spinBox.isChoosable(), false);
...
void TestParameters::test_DoubleSpinBox_WithValidParameters_ShouldConstruct() {
  ...
  /* testing float */ {
    ...
    QCOMPARE(doubleSpinBox.payload(),
             Utils::quoted(Parameters::Detail::MinValue) + ": " + QString::number(0, 'f', 6) +
                 ", " + Utils::quoted(Parameters::Detail::MaxValue) + ": " +
                 QString::number(10, 'f', 6) + ", " + Utils::quoted(Parameters::Detail::Precision) +
                 ": " + QString::number(6));
  ...
  /* testing double */ {
    ...
    QCOMPARE(doubleSpinBox.description(), description);
    QCOMPARE(doubleSpinBox.payload(),
             Utils::quoted(Parameters::Detail::MinValue) + ": " + QString::number(-10, 'f', 2) +
                 ", " + Utils::quoted(Parameters::Detail::MaxValue) + ": " +
                 QString::number(20, 'f', 2) + ", " + Utils::quoted(Parameters::Detail::Precision) +
                 ": " + QString::number(2));
    QCOMPARE(doubleSpinBox.isChoosable(), false);
    QCOMPARE(doubleSpinBox.update("10"), true);
    QCOMPARE(real, 10);
...
void TestParameters::test_DoubleSpinBox_WithInvalidParameters_ShouldThrowException() {
  using namespace Parameters;
...
void TestParameters::test_MappedAngleInRadiansToDegrees_WithValidParameters_ShouldConstruct() {
  ...
  /* testing float */ {
    ...
    QCOMPARE(mappedAngle.payload(),
             Utils::quoted(Parameters::Detail::MinValue) + ": " + QString::number(0.0, 'f', 2) +
                 ", " + Utils::quoted(Parameters::Detail::MaxValue) + ": " +
                 QString::number(360.0, 'f', 2) + ", " +
                 Utils::quoted(Parameters::Detail::Precision) + ": " + QString::number(2));
...
void TestParameters::test_ComboBox_WithValidParameters_ShouldConstruct() {
  ...
  /* testing enum */ {
    ...
    QCOMPARE(comboBox.payload(),
             Utils::quoted(Parameters::Detail::Options) + R"_(: ["A", "B", "C"])_");
  ...
  /* testing QString */ {
    ...
    QCOMPARE(comboBox.description(), description);
    QCOMPARE(comboBox.payload(),
             Utils::quoted(Parameters::Detail::Options) + R"_(: ["G", "H", "I"])_");
    QCOMPARE(comboBox.isChoosable(), true);
  ...
  /* testing int */ {
    ...
    QCOMPARE(comboBox.description(), description);
    QCOMPARE(comboBox.payload(),
             Utils::quoted(Parameters::Detail::Options) + R"_(: [11, 12, 13, 14])_");
    QCOMPARE(comboBox.isChoosable(), true);
...
void TestParameters::test_ComboBox_WithInvalidParameters_ShouldThrowException() {
  using namespace Parameters;
...
void TestParameters::test_MappedComboBox_WithValidParameters_ShouldConstruct() {
  ...
  /* testing enum */ {
    ...
    Arg<Tp> myEnum = Tp::A;
    auto mappedComboBox =
        MappedComboBox(myEnum, {{Tp::A, "HAHA"}, {Tp::B, "HEHE"}, {Tp::C, "HIHI"}}, description);
    ...
    QCOMPARE(mappedComboBox.payload(),
             Utils::quoted(Parameters::Detail::Options) + R"_(: ["HAHA", "HEHE", "HIHI"])_");
  ...
  /* testing int */ {
    Arg<int> integer = 11;
    auto mappedComboBox =
        MappedComboBox(integer, {{11, "11onze"}, {12, "12doze"}, {13, "13treze"}}, description);
    QCOMPARE(mappedComboBox.value(), Utils::quoted("11onze"));
    QCOMPARE(mappedComboBox.type(), "mapped(" + Utils::nameOfType<int>() + ")");
    QCOMPARE(mappedComboBox.inputType(), InputType::ComboBox);
    QCOMPARE(mappedComboBox.description(), description);
    QCOMPARE(mappedComboBox.payload(),
             Utils::quoted(Parameters::Detail::Options) + R"_(: ["11onze", "12doze", "13treze"])_");
    QCOMPARE(mappedComboBox.isChoosable(), true);
...
void TestParameters::test_MappedComboBox_WithInvalidParameters_ShouldThrowException() {
  using namespace Parameters;
  ...
  /* testing enum out of collection */ {
    ...
    bool error = false;
    try {
      auto mappedComboBox = MappedComboBox(myEnum, {{Tp::B, "BLUE"}, {Tp::C, "COLOR"}});
    } catch (...) {
      error = true;
    }
    QVERIFY(error);
  ...
  /* testing int out of collection */ {
    ...
    bool error = false;
    try {
      auto mappedComboBox = MappedComboBox(integer, {{1, "one"}, {2, "two"}, {3, "three"}});
    } catch (...) {
      error = true;
    }
    QVERIFY(error);
...
void TestParameters::test_PushButton_WithValidParameters_ShouldConstruct() {
  ...
  QCOMPARE(pushButton.payload(),
           Utils::quoted(Parameters::Detail::Parent) + ": " +
...
void TestParameters::test_ParametersHandler_WithValidParametersShouldConstruct() {
  ...
  h["enumeration"] = ComboBox(args.enumeration, MagicEnum::values<Args::Enum>());

  h["enumeration2"] = MappedComboBox(args.enumeration2, MagicEnum::entries<Args::Enum2>());
  h["enumeration2"]["B"]["nested"] = DoubleSpinBox(args.nested, 0, 10);

  h["first-layer"]["integer"] = SpinBox(args.firstLayer.integer, 0, 20);
  h["first-layer"]["real"] = DoubleSpinBox(args.firstLayer.real, 0.0, 2.0 * std::acos(-1.0), 6);
