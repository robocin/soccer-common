...
namespace Geometry {
  ...
  constexpr double radiansToDegrees(T radians) {
   ...
   */
  template <std::floating_point T>
  constexpr T smallestAngleDiff(const T& lhs, const T& rhs) {
   ...
   */
  template <std::floating_point T>
  constexpr T normalizeInPI(const T& radians) {
   ...
   */
  template <std::floating_point T>
  constexpr T normalizeIn180(const T& degrees) {
   ...
   */
  template <std::integral T>
  constexpr double normalizeIn180(const T& degrees) {
...
namespace Geometry2D {
  ...
  template <class T>
  concept ConceptOfPoint = is_pt_v<T>;

  template <class T>
  concept ConceptOfPointFloat = ConceptOfPoint<T>&& std::floating_point<CoordType<T>>;

  template <class T>
  concept ConceptOfPointIntegral = ConceptOfPoint<T>&& std::integral<CoordType<T>>;

  /*!
   * @return Returns a PT with the given angle and unitary length.
   */
  template <ConceptOfPoint PT>
  ...
  constexpr PT rotateCW(const PT& p, CoordType<PT> t) {
   ...
   */
  template <ConceptOfPointFloat PT>
  constexpr PT rotateCCW(const PT& p, CoordType<PT> t) {
    return PT(p.x * std::cos(t) - p.y * std::sin(t), p.x * std::sin(t) + p.y * std::cos(t));
  }
   ...
   */
  template <ConceptOfPointFloat PT>
  constexpr PT resize(const PT& p, CoordType<PT> t) {
   ...
   */
  template <ConceptOfPointIntegral PT>
  constexpr PT normalize(const PT& p) {
   ...
   */
  template <ConceptOfPoint PT, class T = CoordType<PT>>
   ...
   */
  template <ConceptOfPointFloat PT>
  PT centroid(const QVector<PT>& polygon) {
    PT ret(0, 0);
   ...
   */
  template <ConceptOfPointFloat PT>
  constexpr PT projectPointLine(const PT& a, const PT& b, const PT& c) {
    if (a == b) {
   ...
   */
  template <ConceptOfPointFloat PT>
  constexpr PT projectPointSegment(const PT& a, const PT& b, const PT& c) {
    if (a == b) {
      throw std::runtime_error("'a' and 'b' doesn't define a line.");
    }
