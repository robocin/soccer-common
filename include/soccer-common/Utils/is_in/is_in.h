...
constexpr std::enable_if_t<detail::is_variant_v<std::decay_t<Variant>>, bool> is_in(T&&, Variant&&);
...
constexpr std::enable_if_t<detail::is_iterable_v<std::decay_t<Container>>, bool> is_in(T&&,
                                                                                       Container&&);
...
constexpr std::enable_if_t<detail::is_tuple_v<std::decay_t<Tuple>>, bool> is_in(T&& lhs,
  ...
  return std::apply(
      [&](auto&&... args) -> bool {
        return (is_in(std::forward<T>(lhs), args) || ...);
      },
...
is_in(T&& lhs, Variant&& variant) {
  return std::visit(
      [&](auto&& rhs) {
        return is_in(std::forward<T>(lhs), rhs);
      },
